/* bat-log.js: anonymous play-style log for NIGHT FLIGHT (P5b, 2026-09-02).
   ...
   Event codes in nl (each event is a small object; n = night):
     ...
     ns  night summary at the end of the night: {k,n,ck,far,pas,brd:[[tier,occupied,kind,id,tr]],
         ...
         disp,wait(waited),why(reason),res(result scene),burn,sav(savior pid),ff(friend-feed pid, P32),e(energy after),
...
var WydLog = (function(){
  ...
  var api = {
    ...
    exit: function(G){
      try{
        if(!G || !G.rlog) return;
        var last = G.rlog.ev[G.rlog.ev.length - 1];
        if(last && last.k === "en") return;   // P32: an Exit on the end screen must not rewrite a finished run as live
      ...
      try{
        ...
        if(id === "dawn" || id === "death"){
          ...
          push(G, {k:"ns", n:G.night, ck: G.cond ? G.cond.key : null, far: R.far ? 1 : 0, pas: R.pasture || null,
            ...
            sav: R.savior || null, ff: R.friendFedPid != null ? R.friendFedPid : null, e: r2(G.energy), sc: Math.round(G.score), tk: G.ticks,
    ...
    sleep: function(G){
      try{
        ...
        api.flush(G, "live");   // P32: without this an abandoned run's record stopped at its last dawn
      }catch(e){}
    },
