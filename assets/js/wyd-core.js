...
/* Which Firestore collection the board reads and posts to. Pages set window.WYD_BOARD before
   loading this file; everything else keeps the original "leaderboard". NIGHT FLIGHT moved to
   "leaderboard_v2" for the 2026-09-04 relaunch and offers the old board read-only in a dropdown
   (window.WYD_BOARD_OLD). Posting always goes to WYD_BOARD. */
var WYD_BOARD = window.WYD_BOARD || "leaderboard";
var WYD_BOARD_OLD = window.WYD_BOARD_OLD || "leaderboard";
var boardSrc = WYD_BOARD;
var FB_CONFIG = {
...
async function loadBoard(){
  if(WYD_DB){
    try{
      var q = WYD_FS.query(WYD_FS.collection(WYD_DB, boardSrc),
...
/* ── end screen + posting ── */
var WYD_FINAL = null;
/* the board's date column: the player's own calendar day (toISOString gave the UTC day, so an evening
   score in the Americas showed up dated tomorrow) */
function wydLocalDate(){ var d = new Date(), p = function(n){ return (n < 10 ? "0" : "") + n; }; return d.getFullYear() + "-" + p(d.getMonth() + 1) + "-" + p(d.getDate()); }
...
function wydWireUi(){
  ...
  if(post) post.addEventListener("click", async function(){
    ...
    var entry = {name: name, sp: WYD_FINAL.sp, score: WYD_FINAL.score,
      date: wydLocalDate(), nights: WYD_FINAL.nights};
    post.disabled = true; post.textContent = "Posting…";
    if(WYD_DB){
      try{
        await WYD_FS.addDoc(WYD_FS.collection(WYD_DB, WYD_BOARD), entry);
  ...
  if(src) src.addEventListener("change", function(){
    boardSrc = src.value === "old" ? WYD_BOARD_OLD : WYD_BOARD;
    var body = $("#wyd-lb-body");
    if(body) body.innerHTML = '<tr><td colspan="6">Loading scores…</td></tr>';
    loadBoard();
  });
  var tabs = $("#wyd-lbtabs");
